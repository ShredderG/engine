...
namespace Sfx {
	...
	void ResetSample(bool restart) {
		...
		square_duty = (float)(0.5 - p_duty * 0.5);
		square_slide = (float)(-p_duty_ramp * 0.00005);

		if (p_arp_mod >= 0.0)
			arp_mod = 1.0 - pow(p_arp_mod, 2) * 0.9;
		else
			arp_mod = 1.0 + pow(p_arp_mod, 2) * 10.0;

		arp_time = 0;
		arp_limit = (int)(pow(1.0 - p_arp_speed, 2) * 20000 + 32);
		if (p_arp_speed == 1.0)
			arp_limit = 0;
		...
		if (!restart) {
			...
			fltp = 0.0;
			fltdp = 0.0;
			fltw = (float)(pow(p_lpf_freq, 3) * 0.1);
			fltw_d = (float)(1.0 + p_lpf_ramp * 0.0001);
			fltdmp = (float)(5.0 / (1.0 + pow(p_lpf_resonance, 2) * 20.0) * (0.01 + fltw));
			if (fltdmp > 0.8) fltdmp = (float)0.8;
			fltphp = 0.0;
			flthp = (float)(pow(p_hpf_freq, 2) * 0.1);
			flthp_d = (float)(1.0 + p_hpf_ramp * 0.0003);

			// reset vibrato
			vib_phase = 0.0;
			vib_speed = (float)(pow(p_vib_speed, 2) * 0.01);
			vib_amp = (float)(p_vib_strength * 0.5);
			...
			for (int i = 0; i < 32; i++)
				noise_buffer[i] = (float)((rand() % 10001) / 10000.0 * 2.0 - 1.0);
	...
	void SynthSample(int length, float * buffer, FILE * file) {
		for (int i = 0; i < length; i++) {
			...
			if (env_stage == 1)
				env_vol = (float)(1.0 + pow(1.0 - (float)env_time / env_length[1], 1) * 2.0 * p_env_punch); // check pow, why 1?
			if (env_stage == 2)
				env_vol = (float)(1.0 - (float)env_time / env_length[2]);
			...
			if (flthp_d != 0.0) {
				flthp *= flthp_d;
				if (flthp < 0.00001) flthp = (float)0.00001;
				if (flthp > 0.1) flthp = (float)0.1;
			}
			...
			for (int si = 0; si < 8; si++) {
				...
				case 0: // square
					...
					break;
				case 1: // sawtooth
					sample = (float)(1.0 - fp * 2.0);
					break;
				case 2: // sine
					sample = (float)sin(fp * PI_DOUBLE);
					break;
				case 3: // noise
			...
			ssample = (float)(ssample / 8.0 * master_vol * 2.0 * sound_vol);
...
char* soundJump = Sfx::create(
	...
	float_to_char((float)0.000000),
	float_to_char((float)0.231340),
	float_to_char((float)0.000000),
	float_to_char((float)0.529980),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.107470),
	float_to_char((float)0.295720),
	float_to_char((float)0.000000),
	false,
	float_to_char((float)0.000000),
	float_to_char((float)1.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.083430),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
	float_to_char((float)0.000000),
